...
//Posiciones bebidas alcoholicas
#define POS_RON "G0 X50"
#define POS_VODKA "G0 X114"
#define POS_GIN "G0 X177"
//#define POS_WHISKY "G0 X239"
//Posiciones refrescos
#define POS_LIMON "G0 X239"

#define POS_PUMP "G0 X350"
//Salidas digitales
...
void ronNaranja(){
  //Movemos pos ron
  strncpy(cmdbuffer[bufindr], POS_RON, sizeof(cmdbuffer[bufindr]));
  SERIAL_ECHO(POS_RON);
  if(Stopped == false) {
    get_coordinates(); // For X Y Z E F
    prepare_move();
  ...
  //levantamos pulsador
  strncpy(cmdbuffer[bufindr], "G0 Y300", sizeof(cmdbuffer[bufindr]));
  SERIAL_ECHO("G0 Z10");
  if(Stopped == false) {
    get_coordinates(); // For X Y Z E F
    prepare_move();
  ...
  strncpy(cmdbuffer[bufindr], "G0 Z650", sizeof(cmdbuffer[bufindr]));
  SERIAL_ECHO("G0 Z50");
  if(Stopped == false) {
    get_coordinates(); // For X Y Z E F
    prepare_move();
  ...
  strncpy(cmdbuffer[bufindr], "G0 Z0", sizeof(cmdbuffer[bufindr]));
  SERIAL_ECHO("G0 Y0");
  if(Stopped == false) {
    get_coordinates(); // For X Y Z E F
    prepare_move();
  ...
  //bajamos pulsador
  strncpy(cmdbuffer[bufindr], "G0 Y0", sizeof(cmdbuffer[bufindr]));
  SERIAL_ECHO("G0 Z0");
  if(Stopped == false) {
    get_coordinates(); // For X Y Z E F
    prepare_move();
  ...
  //Movemos bomba
   strncpy(cmdbuffer[bufindr], POS_PUMP, sizeof(cmdbuffer[bufindr]));
  SERIAL_ECHO(POS_PUMP);
  if(Stopped == false) {
    get_coordinates(); // For X Y Z E F
    prepare_move();
    ClearToSend();
  ...
  fanSpeed=255;
  //Esperamos
  strncpy(cmdbuffer[bufindr], "G0 Z50", sizeof(cmdbuffer[bufindr]));
  SERIAL_ECHO("G0 Z50");
  if(Stopped == false) {
    get_coordinates(); // For X Y Z E F
    prepare_move();
  ...
  //digitalWrite(rele1, LOW);   // Apaga el pin 11
  fanSpeed=0;
  //Movemos inicio
  strncpy(cmdbuffer[bufindr], "G28 X", sizeof(cmdbuffer[bufindr]));
  SERIAL_ECHO("G0 X");
  if(Stopped == false) {
    get_coordinates(); // For X Y Z E F
    prepare_move();
...
void vodkaLimon(){
  //Movemos pos vodka
  strncpy(cmdbuffer[bufindr], POS_VODKA, sizeof(cmdbuffer[bufindr]));
  SERIAL_ECHO(POS_RON);
  if(Stopped == false) {
    get_coordinates(); // For X Y Z E F
    prepare_move();
 ...
 strncpy(cmdbuffer[bufindr], "G0 Y0", sizeof(cmdbuffer[bufindr]));
  ...
  //Movemos limon
  strncpy(cmdbuffer[bufindr], POS_LIMON, sizeof(cmdbuffer[bufindr]));
  SERIAL_ECHO(POS_LIMON);
  if(Stopped == false) {
    get_coordinates(); // For X Y Z E F
    prepare_move();
    ClearToSend();
...
void ginLimon(){
  //pos ginebra
  strncpy(cmdbuffer[bufindr], POS_GIN, sizeof(cmdbuffer[bufindr]));
  SERIAL_ECHO(POS_GIN);
  if(Stopped == false) {
    get_coordinates(); // For X Y Z E F
    prepare_move();
